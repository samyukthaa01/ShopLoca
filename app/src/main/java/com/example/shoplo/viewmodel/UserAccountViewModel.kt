...
import com.example.shoplo.ShopLocaApplication
...
class UserAccountViewModel @Inject constructor(
    ...
    private val _updateInfo = MutableStateFlow<Resource<Seller>>(Resource.Unspecified())
    ...
    fun getUser() {
        ...
        firestore.collection("seller").document(auth.uid!!).get()
    ...
    fun updateUser(user: Seller, imageUri: Uri?) {
        ...
        Log.d("UpdateUser", "areInputsValid: $areInputsValid")
        Log.d("UpdateUser", "imageUri: $imageUri")
    ...
    private fun saveUserInformationWithNewImage(user: Seller, imageUri: Uri) {
        viewModelScope.launch {
            try {
                val imageBitmap = MediaStore.Images.Media.getBitmap(
                    getApplication<ShopLocaApplication>().contentResolver,
                    imageUri
                )
    ...
    private fun saveUserInformation(user: Seller, shouldRetrievedOldImage: Boolean) {
        firestore.runTransaction { transaction ->
            val documentRef = firestore.collection("user").document(auth.uid!!)
            if (shouldRetrievedOldImage) {
                val currentUser = transaction.get(documentRef).toObject(Seller::class.java)
